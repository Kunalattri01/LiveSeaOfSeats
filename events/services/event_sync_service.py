...
def sync_event(api_event):
    ...
    if not start_date:
        print("Skipping event (no start date):", api_event.get("name"))
        return

    # ✅ FIXED UNIQUE LOGIC (NO external_id, using attraction + date)
    ...
    show_start_date = None

    if showtime_date_str and showtime_time_str:
        show_start_date = datetime.strptime(
            f"{showtime_date_str} {showtime_time_str}",
            "%Y-%m-%d %H:%M:%S"
        )
    elif showtime_date_str:
        show_start_date = datetime.strptime(showtime_date_str, "%Y-%m-%d")

    show_date = show_start_date.date() if show_start_date else None
    start_time = show_start_date.time() if show_start_date and showtime_time_str else None
    ...
    for tag_name in tag_names:
        ...
        # ✅ CACHE USED HERE
        if slug_val in TAG_CACHE:
            tag = TAG_CACHE[slug_val]
        else:
    ...
    if locale:
        if locale in LANG_CACHE:
            lang = LANG_CACHE[locale]
        else:
